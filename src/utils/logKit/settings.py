...
def validate_level(level: str) -> LogLevel:
    if level not in ALLOWED_LEVELS:
        msg = f"Level {level} is not allowed"
...
def change_settings(new_root: Path | None = None, new_logs_dir: Path | None = None,
    ...
    if new_logging_config_json:
        logging_config_json = define_setting(new_logging_config_json, validator=validate_path_file)

    if new_setup_logger_name:
        setup_logger_name = new_setup_logger_name

    if new_setup_logger_level:
        setup_logger_level = define_setting(new_setup_logger_level, validator=validate_level)

    if new_default_logger_level:
        default_logger_level = define_setting(new_default_logger_level, validator=validate_level)
